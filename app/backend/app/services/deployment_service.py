        ...
        try:
            async with client.ApiClient() as api:
                ...
                namespace = (
                    ...
                    else (deployment.user_namespace or deployment.namespace)
                )
                ...
                if deployment.is_sso:
                    ...
                    env_vars.append(
                        client.V1EnvVar(name="domain", value="${OIDC_DOMAIN}")
                    )
                    ...
                    volumes.append(
                        client.V1Volume(
                            ...
                            csi=client.V1CSIVolumeSource(
                                driver="csi.ezdata.io", read_only=True
                            ),
                        )
                    )
                    volume_mounts.append(
                        client.V1VolumeMount(
                            name="datasources", mount_path="/mnt/datasources"
                        )
                    )
                ...
                if deployment.is_shared_volume:
                    volumes.append(
                        client.V1Volume(
                            name="ezpresto-shared-pvc",
                            persistent_volume_claim=client.V1PersistentVolumeClaimVolumeSource(
                                claim_name="ezpresto-shared-pvc"
                            ),
                        )
                    )
                    volume_mounts.append(
                        client.V1VolumeMount(
                            name="ezpresto-shared-pvc", mount_path="/mnt/shared"
                        )
                    )
                ...
                if deployment.is_gpu or (
                    deployment.resources and deployment.resources.gpu
                ):
                ...
                container = client.V1Container(
                    ...
                    ports=(
                        [client.V1ContainerPort(container_port=deployment.port)]
                        if deployment.port
                        else None
                    ),
                    ...
                    spec=client.V1DeploymentSpec(
                        ...
                        template=client.V1PodTemplateSpec(
                            ...
                            spec=client.V1PodSpec(
                                ...
                                priority_class_name=(
                                    "vendor-gpu" if deployment.is_gpu else None
                                ),
                            ),
                        ),
                    ),
                ...
                if deployment.expose_service and deployment.port:
                    ...
                    try:
                        config_map = await core_v1.read_namespaced_config_map(
                            name="ezua-cluster-config", namespace="ezua-system"
                        )
                        domain = (
                            config_map.data.get("cluster.domainName")
                            if config_map.data
                            else None
                        )
                    except Exception as e:
                        logger.warning(
                            f"Could not retrieve domain from cluster config: {e}"
                        )
                    ...
                    try:
                        ...
                        logger.info(
                            f"Service {deployment.name} created in namespace {namespace}"
                        )
                    ...
                    if domain:
                        ...
                        try:
                            ...
                            if e.status != 409:
                                ...
                                logger.info(
                                    f"VirtualService {deployment.name}-vs already exists"
                                )
                        ...
                        try:
                            ...
                            logger.info(
                                f"AuthorizationPolicy {deployment.name}-auth-policy created"
                            )
                        except client.exceptions.ApiException as e:
                            if e.status != 409:
                                logger.warning(
                                    f"Failed to create AuthorizationPolicy: {e}"
                                )
                            else:
                                logger.info(
                                    f"AuthorizationPolicy {deployment.name}-auth-policy already exists"
                                )
