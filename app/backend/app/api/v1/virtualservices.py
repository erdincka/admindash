...
async def list_virtualservices(
    ...
    try:
        ...
        async with client.ApiClient() as api:
            ...
            return ApiResponse(
                success=True, data=vs_list.get("items", []), timestamp=datetime.utcnow()
            )
    ...
    try:
        ...
        if not all([namespace, service_name, hostname, domain]):
            ...
            raise ValidationError(
                "namespace, service_name, hostname, and domain are required"
            )
        ...
        async with client.ApiClient() as api:
            ...
            try:
                ...
                if e.status == 409:
                    logger.warning(
                        f"AuthorizationPolicy {hostname}-auth-policy already exists"
                    )
                else:
                    raise
