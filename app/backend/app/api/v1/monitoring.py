    ...
    try:
        async with httpx.AsyncClient() as client:
            response = await client.get(
                f"{PROMETHEUS_URL}/api/v1/query", params=params, timeout=10.0
            )
            ...
            if response.status_code != 200:
                ...
                raise HTTPException(
                    ...
                    detail="Failed to query Prometheus",
                )
            ...
            if data.get("status") != "success":
                raise HTTPException(
                    status_code=500,
                    detail=data.get("error", "Unknown Prometheus error"),
                )

            return ApiResponse(
                success=True, data=data.get("data"), timestamp=datetime.utcnow()
            )

    except httpx.RequestError as exc:
        logger.error(f"Connection error to Prometheus: {exc}")
        raise HTTPException(
            status_code=500, detail=f"Connection error to Prometheus: {str(exc)}"
        )
