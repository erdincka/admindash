...
async def list_resources(
    ...
    if kind.lower() not in SUPPORTED_KINDS:
        ...
        raise ValidationError(
            f"Unsupported resource kind: {kind}. Supported: {', '.join(SUPPORTED_KINDS)}"
        )
...
async def get_resource(
    ...
    return ApiResponse(success=True, data=resource_dict, timestamp=datetime.utcnow())
...
async def get_resource_dependencies(
    ...
    try:
        ...
        dependencies = {"upstream": [], "downstream": [], "related": []}

        async with client.ApiClient() as api:
            # First, get the resource itself to check its spec/metadata
            resource = None
            if kind == "pod":
                resource = await client.CoreV1Api(api).read_namespaced_pod(
                    name, namespace
                )
            elif kind == "deployment":
                resource = await client.AppsV1Api(api).read_namespaced_deployment(
                    name, namespace
                )
            elif kind == "service":
                resource = await client.CoreV1Api(api).read_namespaced_service(
                    name, namespace
                )
            elif kind == "replicaset":
                resource = await client.AppsV1Api(api).read_namespaced_replica_set(
                    name, namespace
                )
            elif kind == "persistentvolumeclaim":
                resource = await client.CoreV1Api(
                    api
            ...
            if not resource:
                ...
                return ApiResponse(
                    success=True, data=dependencies, timestamp=datetime.utcnow()
                )
            ...
            if kind == "service":
                ...
                if selector:
                    ...
                    all_pods = await client.CoreV1Api(api).list_namespaced_pod(
                        namespace
                    )
            ...
            # DEPLOYMENT -> REPLICASETS (Downstream)
                ...
                if selector:
                    all_rs = await client.AppsV1Api(api).list_namespaced_replica_set(
                        namespace
                    )
        ...
        logging.getLogger(__name__).error(
            f"Error getting dependencies: {e}", exc_info=True
        )
...
async def get_resource_events(
    ...
    try:
        ...
        async with client.ApiClient() as api:
            ...
            events_list = await v1.list_namespaced_event(
            ...
            logger = logging.getLogger(__name__)
            logger.info(
                f"Found {len(events_list.items)} events for {kind}/{name} in {namespace}"
            )
            ...
            for event in events_list.items:
                ...
                if (
                    ...
                    and obj.name == name
                    and obj.kind
                    and obj.kind.lower() == target_kind
                ):
                    ...
                    logger.debug(
                        f"Skipping event {event.metadata.name}: obj.name={obj.name if obj else 'None'}, obj.kind={obj.kind if obj else 'None'}"
                    )

            # Sort by last_timestamp desc
            related_events.sort(
                key=lambda x: x["last_timestamp"]
                or x["first_timestamp"]
                or datetime.min,
                reverse=True,
            )

            return ApiResponse(
                success=True, data=related_events, timestamp=datetime.utcnow()
            )
    ...
    try:
        ...
        async with client.ApiClient() as api:
            ...
            if kind == "pod":
                ...
                resource = await api_instance.read_namespaced_deployment(
                    name, namespace
                )
            elif kind == "service":
                api_instance = client.CoreV1Api(api)
                ...
                api_instance = client.CoreV1Api(api)
                resource = await api_instance.read_namespaced_config_map(
                    name, namespace
                )
            elif kind == "secret":
                api_instance = client.CoreV1Api(api)
                ...
                api_instance = client.CoreV1Api(api)
                resource = await api_instance.read_namespaced_persistent_volume_claim(
                    name, namespace
                )
                ...
                api_instance = client.AppsV1Api(api)
                resource = await api_instance.read_namespaced_stateful_set(
                    name, namespace
                )
            elif kind == "daemonset":
                api_instance = client.AppsV1Api(api)
                resource = await api_instance.read_namespaced_daemon_set(
                    name, namespace
                )
            elif kind == "replicaset":
                api_instance = client.AppsV1Api(api)
                resource = await api_instance.read_namespaced_replica_set(
                    name, namespace
                )
            ...
            return ApiResponse(
                success=True, data=yaml_content, timestamp=datetime.utcnow()
            )
...
async def apply_yaml(
    ...
    try:
            ...
            for idx, manifest in enumerate(manifests):
                try:
                    ...
                    if not isinstance(manifest, dict):
                        raise ValidationError(
                            f"Manifest {idx + 1} must be a dictionary"
                        )

                    if "apiVersion" not in manifest:
                        raise ValidationError(
                            f"Manifest {idx + 1} missing 'apiVersion' field"
                        )

                    if "kind" not in manifest:
                        raise ValidationError(
                            f"Manifest {idx + 1} missing 'kind' field"
                        )

                    if "metadata" not in manifest or "name" not in manifest.get(
                        "metadata", {}
                    ):
                        raise ValidationError(
                            f"Manifest {idx + 1} missing 'metadata.name' field"
                        )
                    ...
                    try:
                        ...
                        created_objects = await utils.create_from_dict(
                            api, manifest, namespace=namespace, verbose=True
                        )

                        logger.info(
                            f"Successfully created {kind}/{name} in namespace {namespace}"
                        )
                    ...
                    logger.error(
                        f"Unexpected error applying manifest {idx + 1}: {e}",
                        exc_info=True,
                    )
                    results.append(
                        {
        ...
        logger.info(
            f"Apply results: {created_count} created, {exists_count} already exist, {failed_count} failed"
        )
    ...
    try:
        ...
        async with client.ApiClient() as api:
            if kind == "pod":
                ...
                await client.CoreV1Api(api).delete_namespaced_config_map(
                    name, namespace
                )
            elif kind == "secret":
                await client.CoreV1Api(api).delete_namespaced_secret(name, namespace)
            elif kind == "persistentvolumeclaim":
                await client.CoreV1Api(api).delete_namespaced_persistent_volume_claim(
                    name, namespace
                )
            elif kind == "persistentvolume":
                await client.CoreV1Api(api).delete_persistent_volume(name)
            elif kind == "statefulset":
                await client.AppsV1Api(api).delete_namespaced_stateful_set(
                    name, namespace
                )
            elif kind == "daemonset":
                await client.AppsV1Api(api).delete_namespaced_daemon_set(
                    name, namespace
                )
            elif kind == "replicaset":
                await client.AppsV1Api(api).delete_namespaced_replica_set(
                    name, namespace
                )
            elif kind == "virtualservice":
