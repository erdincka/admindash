    ...
    try:
        ...
        async with httpx.AsyncClient() as client:
            response = await client.get(
                "http://chartmuseum.ez-chartmuseum-ns.svc.cluster.local:8080/api/charts"
            )

            if response.status_code != 200:
                logger.error(
                    f"Failed to fetch charts from ChartMuseum: {response.text}"
                )
        ...
        return ApiResponse(success=True, data=releases, timestamp=datetime.utcnow())
...
async def install_chart(request: HelmInstallRequest):
