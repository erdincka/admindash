...
async def browse_storage(
    ...
    params = {"list-type": "2", "delimiter": "/", "prefix": request.path}
    ...
    try:
        async with httpx.AsyncClient(verify=False) as client:
            response = await client.get(
                endpoint, params=params, headers=headers, timeout=10.0
            )

            if response.status_code != 200:
                raise HTTPException(
                    status_code=response.status_code,
                    detail=f"S3 Error: {response.text}",
                )
            ...
            for content in root.findall(
                ...
                ) or content.find("Size")
                last_mod = content.find(
                    "{http://s3.amazonaws.com/doc/2006-03-01/}LastModified"
                ) or content.find("LastModified")
                ...
                if key is not None and key.text:
                    ...
                    files.append(
                        FileItem(
                            ...
                            last_modified=(
                                last_mod.text if last_mod is not None else None
                            ),
                        )
                    )
            ...
            return ApiResponse(
                success=True, data=list(unique_files), timestamp=datetime.utcnow()
            )
...
async def list_filesystem(
    ...
    return ApiResponse(success=True, data=files, timestamp=datetime.utcnow())
